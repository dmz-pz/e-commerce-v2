      ...
      <Link to={`/product/${product.id}`} className="flex flex-col flex-1 min-h-0">
        ...
        <div className="flex flex-col flex-1 mt-3 md:mt-4 min-h-0">
          <div className="flex justify-between items-start mb-1 md:mb-2 gap-2 h-8 md:h-10 shrink-0">
            <h3 className="font-bold text-slate-900 text-[11px] md:text-sm tracking-tight leading-tight flex-1 group-hover:text-brand transition-colors line-clamp-2">{product.name}</h3>
            <div className="flex flex-col items-end leading-none shrink-0">
              {product.discountPrice ? (
                <>
                  <span className="text-sm md:text-base font-black text-brand tracking-tighter">${product.discountPrice.toFixed(2)}</span>
                  <span className="text-[8px] md:text-[10px] font-bold text-slate-300 line-through tracking-tighter">${product.price.toFixed(2)}</span>
                </>
              ) : (
                <span className="text-sm md:text-base font-black text-brand tracking-tighter">${product.price.toFixed(2)}</span>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-1.5 md:gap-2 items-center mb-2 shrink-0">
            ...
            <span className={`text-[8px] md:text-[10px] px-1.5 md:px-2 py-0.5 rounded-full font-bold uppercase tracking-tighter ${
              product.stock < 10 ? 'bg-orange-50 text-orange-600' : 'bg-brand/10 text-brand'
            }`}>
              St: {product.stock}
            </span>
          </div>
      ...
      <div className="mt-auto">
        {currentQuantity > 0 ? (
          <div className="flex items-center bg-brand rounded-lg md:rounded-xl overflow-hidden shadow-lg shadow-brand/10">
            ...
            <button 
              onClick={() => updateQuantity(product.id, Math.min(product.stock, currentQuantity + 1))}
