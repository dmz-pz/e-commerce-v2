...
export const SubstitutionModal: React.FC<SubstitutionModalProps> = ({
  ...
  React.useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      setLoading(true);
      productService.getProducts({ search: searchQuery, limit: 15 })
        .then(data => {
          // Removemos de los resultados el mismo producto que estamos intentando sustituir
          const results = (data.items || []).filter(p => p.id !== substitutingItem.productId && p.isActive !== false);
    ...
    <AnimatePresence>
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" id="substitution-overlay">
        ...
        >
          ...
          {/* Lista de Candidatos */}
          <div className="flex-1 overflow-y-auto p-5 space-y-2.5 no-scrollbar min-h-[200px]">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-full text-brand mt-10">
                <Loader2 className="w-8 h-8 animate-spin mb-3" />
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Buscando productos...</span>
              </div>
            ) : (
              <>
                ...
                {!searchQuery && searchResults.length === 0 && (
                  <div className="text-center py-8 text-slate-400 font-bold uppercase tracking-widest text-[10px]">
                    Escribe para buscar un sustituto...
                  </div>
                )}
              </>
            )}
