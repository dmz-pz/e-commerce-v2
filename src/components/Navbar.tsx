...
export const Navbar: React.FC = () => {
  ...
  const hideCategoryNavPaths = ["/checkout", "/login", "/register", "/forgot-password", "/reset-password"];
  const shouldHideCategoryNav = hideCategoryNavPaths.includes(location.pathname);
  ...
  return (
    <>
          ...
          <div className="max-w-[1920px] mx-auto px-4 md:px-8 w-full">
            <div className="flex justify-between items-center gap-4 md:gap-8">
              ...
              <div className="flex items-center gap-2 md:gap-4 shrink-0">
                {/* Tasa del Día Badge */}
                <div className="flex flex-col items-end justify-center px-1 md:px-2 border-r border-white/20 md:border-slate-200 mr-1 md:mr-2 pr-2 md:pr-4">
                  <span className="text-[7px] md:text-[8px] font-bold text-white/70 md:text-slate-400 uppercase tracking-[0.2em] mb-0.5">Tasa del día</span>
                  <span className="text-[10px] md:text-sm font-black text-accent md:text-brand tracking-widest leading-none">
            ...
            {selectedCategory === "all" && !shouldHideCategoryNav && (
      ...
      <div className={`${selectedCategory === "all" && !shouldHideCategoryNav ? "h-[124px]" : "h-[64px]"} ${shouldHideCategoryNav ? "md:h-20" : "md:h-[128px]"} pointer-events-none shrink-0`} />
    </>
  );
};
