...
import { ShoppingCart, Package, User, Search, ShoppingBasket, ChevronDown, ShieldAlert, Bike } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const isStaffOrAdmin = user && (user.role === 'staff' || user.role === 'picker' || user.role === 'admin' || user.role === 'delivery');
            ...
            <div className="max-w-[1920px] mx-auto px-4 md:px-8 w-full flex justify-between items-center">
              ...
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <span className="font-black text-base md:text-xl tracking-tight text-white md:text-brand uppercase">
                  Mi<span className="text-accent underline decoration-white md:decoration-brand underline-offset-4">negocio</span>
                </span>
        ...
        <nav className="bg-brand md:bg-white border-b border-white/10 md:border-slate-100 min-h-[64px] md:h-20 flex items-center shadow-sm relative z-20">
          <div className="max-w-[1920px] mx-auto px-4 md:px-8 w-full">
            <div className="flex justify-between items-center gap-4 md:gap-8">
              ...
              <Link to="/" className="flex items-center gap-2 md:gap-3 shrink-0 absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0">
                <Logo className="w-10 h-10 md:w-12 md:h-12 drop-shadow-md" />
              ...
              <div className="flex items-center gap-2 md:gap-4 shrink-0">
                <div className="hidden lg:flex items-center gap-6 mr-4">
                  {user && (user.role === 'staff' || user.role === 'picker' || user.role === 'admin') && (
                    <Link to="/staff" className="text-[10px] font-black text-slate-400 hover:text-brand transition-colors uppercase tracking-widest flex items-center gap-2">
                      <Package className="w-4 h-4" />
                      Operaciones
                    </Link>
                  )}
                  {user && (user.role === 'delivery' || user.role === 'admin') && (
                    <Link to="/delivery" className="text-[10px] font-black text-slate-400 hover:text-brand transition-colors uppercase tracking-widest flex items-center gap-2">
                      <Bike className="w-4 h-4" />
                      Reparto
                    </Link>
                  )}
                  {user && user.role === 'admin' && (
                    <Link to="/admin" className="text-[10px] font-black text-slate-400 hover:text-brand transition-colors uppercase tracking-widest flex items-center gap-2">
                ...
                >
                  <User className={`w-5 h-5 transition-colors ${user ? 'text-brand' : 'text-slate-400'} group-hover:text-brand`} />
                </Link>
              </div>
            </div>
            ...
            <div className="max-w-[1920px] mx-auto px-4 md:px-8 w-full h-full flex items-center">
              <div className="flex items-center justify-center gap-2 w-full">
                <div className="hidden lg:flex items-center gap-1 bg-white/10 px-4 py-1.5 rounded-lg border border-white/10 mr-4 shrink-0 cursor-pointer hover:bg-white/20 transition-colors">
                  ...
                  <span className="text-[10px] font-black text-white uppercase tracking-wider">Categorías</span>
                ...
                <div className="flex items-center gap-1 overflow-x-auto no-scrollbar justify-start md:justify-center">
                  {categories.map((cat) => (
                    <button
                      ...
                      className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all rounded-lg ${
